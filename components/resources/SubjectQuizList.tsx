...
import { SubjectQuizCard } from "./SubjectQuizCard";
...
export function SubjectQuizList() {
  const [quizzes, setQuizzes] = useState<SubjectQuizInfo[]>([]);
    ...
    return (
      <div className="rounded-xl border border-taupe/30 p-8 text-center space-y-3">
        <p className="text-sm text-ink-muted">
          Complete modules in a subject to unlock practice quizzes.
        </p>
        <p className="text-xs text-ink-faint">
          Quizzes are generated from the lessons in completed modules.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      {availableQuizzes.length > 0 && (
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="font-medium text-ink">Subject Quizzes ({availableQuizzes.length})</h4>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {availableQuizzes.map((quiz) => (
              <SubjectQuizCard key={quiz.subjectId} info={quiz} />
            ))}
          </div>
        </section>
      )}
      ...
      {unavailableQuizzes.length > 0 && (
        <section className="space-y-3">
          ...
          <p className="text-xs text-ink-faint">
            These subjects don&apos;t have enough extractable content for quizzes.
