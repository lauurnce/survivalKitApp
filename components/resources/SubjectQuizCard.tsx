...
export function SubjectQuizCard({ info }: SubjectQuizCardProps) {
  ...
  return (
    <Link
      href={`/quiz/subject/${info.subjectId}`}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-ink-muted mt-1">
            {info.completedModules} of {info.totalModules} modules completed
          </p>
        </div>
      ...
      {!info.hasQuizMaterial && (
        <div className="mt-3 pt-3 border-t border-taupe/20 text-xs text-ink-muted">
          This subject doesn&apos;t have enough content for a quiz yet.
        </div>
      )}
    </Link>
  );
