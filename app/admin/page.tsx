...
export default async function AdminPage({ searchParams }: Props) {
  ...
  if (!(await checkAdminAuth(sp))) return <AdminLogin />;
  ...
  const [
    ...
    supabase.from("events").select("device_id, created_at")
      .eq("event_type", "enter")
      .gte("created_at", thirtyDaysAgo)
      .order("created_at", { ascending: true }),
    supabase.from("events").select("subject_id, subjects(title)")
      .eq("event_type", "subject_open").not("subject_id", "is", null).limit(1000),
    supabase.from("events").select("module_id, modules(title)")
      .eq("event_type", "module_open").not("module_id", "is", null).limit(1000),
    supabase.from("events").select("section_id")
      .eq("event_type", "section_view").not("section_id", "is", null).limit(3000),
    supabase.from("unlocks").select("id, device_id, gcash_ref, amount, created_at, modules(title)")
      .eq("status", "pending").order("created_at", { ascending: false }).limit(50),
    supabase.from("unlocks").select("id").eq("status", "approved"),
  ]);

  // Funnel: unique device_ids per event_type
  const allEvents = funnelRaw ?? [];
  const funnel = FUNNEL_STEPS.map(step => ({
    type: step.type,
    label: step.label,
    hint: step.hint,
    unique: new Set(allEvents.filter(e => e.event_type === step.type).map(e => e.device_id)).size,
  }));
  ...
  const topSubjects = countByLabel(topSubjectsRaw ?? [], s => getTitle((s as { subjects: unknown }).subjects));
  const topModules  = countByLabel(topModulesRaw ?? [],  m => getTitle((m as { modules: unknown }).modules));

  // Top sections: count by section_id, then fetch headings
  const sectionCounts = countById((sectionEventRaw ?? []).map(e => (e as { section_id: string | null }).section_id));
